...
pub fn execute(command: Command, app: &mut App, theme: &Theme) -> Result<()> {
    match command {
        ...
        Command::EditSelected => open_editor(app, theme),
...
/// backed by `tui-textarea`). Does nothing for directories, and for
/// files that fail to load as UTF-8 text (binary files aren't
/// supported yet — see `TODO.md`) rather than crashing the app.
fn open_editor(app: &mut App, theme: &Theme) {
    ...
    if let Ok(editor) = Editor::open(path, theme) {
